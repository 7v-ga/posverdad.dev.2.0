...
-- 2) Métricas por medio y mes (derivadas en SQL)
WITH base AS (
  SELECT
    s.name AS source,
    date_trunc('month', a.publication_date::timestamp) AS month,
    CASE
      WHEN a.polarity IS NULL THEN 'not_calculated'
      WHEN a.polarity = -1 THEN 'negative'
      WHEN a.polarity =  0 THEN 'neutral'
...
-- 3) Matriz polarity × subjectivity_bucket últimos 30 días (SQL)
SELECT
  CASE
